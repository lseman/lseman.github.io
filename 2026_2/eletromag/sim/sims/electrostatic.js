...
<div class="legend"><span class="legend-item"><span class="legend-dot" style="background:#f87171"></span>+ Positiva</span><span class="legend-item"><span class="legend-dot" style="background:#38bdf8"></span>− Negativa</span></div>`;
	...
	render(c, time) {
		...
		if(this.showProbe){const p=new V(S.mouse.x,S.mouse.y),e=this.E(p),v=this.V(p);c.fillStyle="rgba(7,10,18,.82)";c.fillRect(12,H-67,285,51);c.fillStyle="#cbd5e1";c.font="10px monospace";c.fillText(`sonda (${p.x.toFixed(0)}, ${p.y.toFixed(0)}) px`,22,H-49);c.fillText(`E = (${e.x.toExponential(2)}, ${e.y.toExponential(2)}) V/m`,22,H-34);c.fillText(`|E| = ${e.len().toExponential(2)} V/m   V = ${v.toFixed(2)} V`,22,H-20)}
	}
}
