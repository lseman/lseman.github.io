...
<div class="legend"><span class="legend-item"><span class="legend-dot" style="background:#818cf8"></span>Equipotenciais</span><span class="legend-item"><span class="legend-dot" style="background:#38bdf8"></span>Campo (⊥)</span></div>`;
	...
	render(c, time) {
		...
		if(this.showProbe){const p=new V(S.mouse.x,S.mouse.y),e=this.E(p),v=this.V(p);const probeW=310,probeH=68,probeX=W-probeW-16,probeY=60;c.fillStyle="rgba(7,10,18,.82)";c.beginPath();c.roundRect(probeX,probeY,probeW,probeH,8);c.fill();c.strokeStyle="rgba(103,232,249,.22)";c.stroke();c.fillStyle="#cbd5e1";c.font="10px monospace";c.fillText(`sonda: V=${v.toFixed(3)} V`,probeX+10,probeY+22);c.fillText(`E=−∇V=(${e.x.toExponential(2)}, ${e.y.toExponential(2)}) V/m`,probeX+10,probeY+40);c.fillText(`|E|=${e.len().toExponential(2)} V/m`,probeX+10,probeY+58)}
	}
	drawContours(c) {
