...
<div class="legend"><span class="legend-item"><span class="legend-dot" style="background:#38bdf8"></span>valor negativo</span><span class="legend-item"><span class="legend-dot" style="background:#e2e8f0"></span>próximo de zero</span><span class="legend-item"><span class="legend-dot" style="background:#fb7185"></span>valor positivo</span></div>`;
	...
	render(c, time) {
		...
		const mx=max(0,min(W,S.mouse.x)),my=max(0,min(H,S.mouse.y)),wx=(mx-W/2)/scale,wy=(H/2-my)/scale,F=this.fieldF(wx,wy),G=this.gradF(wx,wy);c.strokeStyle="rgba(103,232,249,.55)";c.lineWidth=1;c.beginPath();c.arc(mx,my,5,0,2*PI);c.moveTo(mx-10,my);c.lineTo(mx-4,my);c.moveTo(mx+4,my);c.lineTo(mx+10,my);c.moveTo(mx,my-10);c.lineTo(mx,my-4);c.moveTo(mx,my+4);c.lineTo(mx,my+10);c.stroke();const probeW=306,probeH=70,probeX=W-probeW-16,probeY=16;c.fillStyle="rgba(7,10,18,.88)";c.beginPath();c.roundRect(probeX,probeY,probeW,probeH,10);c.fill();c.strokeStyle="rgba(103,232,249,.22)";c.stroke();c.fillStyle="#67e8f9";c.font="600 10px monospace";c.fillText(`SONDA  x ${wx.toFixed(2)}  y ${wy.toFixed(2)}`,probeX+10,probeY+22);c.fillStyle="#cbd5e1";c.font="10px monospace";c.fillText(`f ${this.scalarF(wx,wy).toFixed(3)}   ∇f (${G.x.toFixed(2)}, ${G.y.toFixed(2)})`,probeX+10,probeY+40);c.fillText(`F (${F.x.toFixed(2)}, ${F.y.toFixed(2)})   div ${this.divF(wx,wy).toFixed(2)}   rot ${this.curlF(wx,wy).toFixed(2)}`,probeX+10,probeY+58);
