...
<div class="stat-grid" id="stats"></div>`;
		...
		el.querySelector("#tp").onchange = (e) => { this.type = e.target.value; el.querySelector("#formula").textContent=`${formulas[this.type]}  |  U = ½CV²`; this.calc(el); };
	...
	calc(el) {
		...
		if (this.type === "parallel") C = (EPS0 * (this.A / 1e6)) / (this.d / 1e3);
			...
			const a = this.a || 15,
				b = this.b || 40;
			C = (2 * PI * EPS0 * (this.d / 1e3)) / log(b / a);
		} else {
			const a = 20,
				b = 50;
			C = (4 * PI * EPS0 * ((a * b) / (b - a))) / 1e3;
		}
		...
		else if (this.type === "cyl") { const a=.015,b=.040; E=this.V/(a*log(b/a)); }
		else { const a=.020,b=.050; E=(this.V*b)/(a*(b-a)); }
